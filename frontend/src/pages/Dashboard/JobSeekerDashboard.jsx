...
//     getAllResources({ limit: 4 }).then((data)=>{
...
export default function JobSeekerDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch applications
    fetch(`${API_BASE}/jobseeker/applications?limit=20`, { 
      headers: { Authorization: `Bearer ${token}` },
      credentials: 'include'
    })
      .then(r=>r.json()).then(d=>{
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border">
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-gray-100 rounded-full h-2">
                  <div 
                    ...
                    style={{width: `${stats.total > 0 ? Math.min(100, (stats.shortlisted + stats.interview) / stats.total * 100) : 0}%`}} 
                  />
                </div>
              ...
              <div className="text-xs text-gray-500 mt-2">
                ...
                {stats.pending > 0 && stats.shortlisted > 0 && ' • '}
                {stats.shortlisted > 0 && `${stats.shortlisted} Shortlisted`}
              ...
              <div className="p-6">
                {recent.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {recent.map(a => {
                      const jobData = a.job || {};
                      const appliedDate = a.appliedAt ? new Date(a.appliedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
                      
                      return (
                        <div key={a._id} className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-gray-900 truncate">{jobData.title || 'Job Title'}</div>
                            <div className="text-sm text-gray-500">
                              {jobData.company || 'Company'} · {jobData.location || 'Location'}
                              {appliedDate && ` · Applied ${appliedDate}`}
                            </div>
                          </div>
                          <span className={`text-xs px-3 py-1 rounded-full capitalize whitespace-nowrap ${
                    ...
                    {resources.map((r) => (
                      <Link key={r._id} to={`/resources/${r._id}`} className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg transition group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500">{r.category?.replace('-', ' ')} Â· {r.type}</div>
