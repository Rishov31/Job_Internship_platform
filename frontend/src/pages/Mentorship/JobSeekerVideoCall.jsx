...
export default function JobSeekerVideoCall() {
    ...
    const loadData = async () => {
      try {
        ...
        if (roomId) {
          ...
          // Determine other user (mentor)
          // The mentor is stored in room.mentor (populated User)
          if (room.jobseeker?._id?.toString() === userId?.toString() || room.jobseeker?.toString() === userId?.toString()) {
  ...
  const handleCallEnd = () => {
    ...
    navigate(`/jobseeker/mentor-chats?room=${selectedRoom?._id}`);
  };
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="w-64 bg-gray-900 text-white flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <Link to="/jobseeker/video-call" className="flex items-center gap-3 px-4 py-3 bg-blue-600 rounded-lg">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {selectedRoom ? (
            ...
            <div className="bg-white border-b p-4 flex-shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium">{otherUser?.fullName || "Mentor"}</div>
                    <div className="text-sm text-gray-500">Video Call</div>
                  </div>
                </div>
                <Link
                  to={`/jobseeker/mentor-chats?room=${selectedRoom._id}`}
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center text-white">
                    <div className="mb-6">
                      ...
                      <p className="text-gray-400">Click the button below to start the call with {otherUser?.fullName || "your mentor"}</p>
                    </div>
                    <button
                      onClick={startCall}
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center text-gray-500">
              ...
              <Link to="/jobseeker/mentor-chats" className="text-blue-600 hover:text-blue-700 mt-4 inline-block">
                Go to Chat
              </Link>
            </div>
          </div>
